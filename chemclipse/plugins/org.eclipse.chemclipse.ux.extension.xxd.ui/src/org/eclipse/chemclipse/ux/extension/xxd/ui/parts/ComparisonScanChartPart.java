...
public class ComparisonScanChartPart extends AbstractDataUpdateSupport implements IDataUpdateSupport {
	...
	private static final TargetExtendedComparator COMPARATOR = new TargetExtendedComparator();
	...
	public void updateObjects(List<Object> objects, String topic) {
			...
			if(isUnloadEvent(topic)) {
				...
				if(object instanceof IScanMSD) {
					...
					target = IIdentificationTarget.getBestIdentificationTarget(scan.getTargets(), COMPARATOR);
				} else if(object instanceof IPeakMSD) {
					IPeakMSD peakMSD = (IPeakMSD)object;
					scan = peakMSD.getExtractedMassSpectrum();
					target = IIdentificationTarget.getBestIdentificationTarget(peakMSD.getTargets(), COMPARATOR);
				} else {
					return;
				}
				if(target != null) {
